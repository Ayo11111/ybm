 ...
 * @LastEditors: CJH
 * @LastEditTime: 2021-09-29 10:35:53
-->
<template>
  <div v-loading="mainLoad || saveLoad" class="container">
    <el-form label-width="7em">
      ...
      <el-form-item label="店铺地址" prop="fullAddress">
        ...
        <span>{{ detail.fullAddress || detail.businessAddress }}</span>
        <el-button class="ml-12" @click="addressFocus">修改</el-button>
      </el-form-item>
      <el-form-item label="配送费">
        <div>
          <el-input-number
            v-model="detail.deliveryPrice"
            :min="0"
            :controls="false"
          />
          <span> 元</span>
      ...
      <el-form-item label="可配送医院">
        <div>
          <b>{{ merchantInfo.hospitalName }}</b>
          <div style="font-size: 13px; color: #7f7f7f; line-height: 1">
            更改配送医院需要联系区域经理进行修改，电话<span v-loading="phoneLoad">{{ phone }}</span>
          </div>
        </div>
      </el-form-item>
      <el-form-item>
...
import { fhGetOption, fhPutDeliveryPrice, fhGetOptionLiaisonPhone } from '@/api/merchant'
export default {
  ...
  methods: {
    ...
    async getOptionLiaisonPhone() {
      this.phoneLoad = true
      try {
        const { results } = await fhGetOptionLiaisonPhone()
        this.phone = results
      } catch (err) {
        console.log(err)
      }
