    ...
    <div v-loading="loading" class="box">
      ...
      <div class="right">
        <div v-if="radioOrcheckbox === 'checkbox'" style="margin-left: 25px">
          ...
          <span style="float: right" class="delete-item" @click="delall">清空</span>
...
import { fhGetStoteFoods } from '@/api/stores/commodity'
  ...
  methods: {
    ...
    changeS(val, item) {
      if (val) {
        ...
        ;``
        const status = this.selected.delete(item)
        this.selectedId.delete(item.id)
        if (!status) {
          const { id } = item
          const selecteArr = Array.from(this.selected)
          const newSelecte = selecteArr.filter(i => i.id !== id)
    ...
    async getFoodList(params, options = {}) {
      ...
      try {
        ...
        const { results } = this.share
          ? await fhGetStoteFoods(await getStoreId(), params)
          : await fhGetStoteFoods(await getStoreId(), params)
