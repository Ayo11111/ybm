  ...
  <div class="wrap">
    ...
    >
      ...
      <div slot="edit" slot-scope="scope, item, index">
        <router-link
          :to="{ name: 'discountDetail', params: { id: scope.activityId } }"
        >
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          title: '折扣菜品',
          dataIndex: 'discountFoodTemplateRspList',
          scopedSlots: { customRender: 'discountFoodTemplateRspList' }
        },
        {
          title: '启\禁用',
          dataIndex: 'upStatus',
  ...
  methods: {
    ...
    async upStatus(row, event) {
      ...
      try {
        await fhStatusDiscount(id, query)
    ...
    async getDiscount() {
      ...
      delete params.date
      try {
        const { results, count } = await fhGetDiscount(params)
