<template>
  <div class="container">
    <div v-loading="loadingSave" class="upload">
      <el-form
        ref="form"
        :model="formData"
        :rules="rules"
      ...
      >
        ...
        <el-form-item label="选择店铺" prop="storeIdList">
          <el-button type="primary" :disabled="false" @click="dialogVisible = true">
            选择店铺
          </el-button>
          <el-button v-if="formData.storeIdList.length > 0" type="text">
            已选择{{ formData.storeIdList.length }}家店铺
          </el-button>
          <el-form-item style="margin-left:68px;margin-top:20px">
            <StoreTag v-if="formData.storeNameList" :store-arr="formData.storeNameList" />
          </el-form-item>
        </el-form-item>
        <el-divider />
          ...
          <el-form-item prop="foodTemplateIdList" :rules="rules.foodTemplateIdList">
            ...
            <div class="commodity">
              ...
              <div style="margin: 8px 8px">
                <el-tag v-for="(item,index) in selectedFood" :key="index" type="info">
                  <template v-if="typeof(item) === 'object'">{{ item.name }}</template>
    ...
    <el-divider />
    <el-button type="primary" :loading="loadingSave" @click="save">保存</el-button>
    <el-button>
    ...
    <SelectStore
      ...
      :checked-store="checkedStore"
      :checked-store-id.sync="formData.storeIdList"
...
import StoreTag from './_components/tag'
...
export default {
  ...
  methods: {
    ...
    save() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (isArray(this.formData.weekCycle)) {
            sendData.weekCycle = this.formData.weekCycle.reverse().join('')
          }
          sendData.actPeriodReqList =
        sendData.actPeriodReqList.length > 0
          ? formatPeriod(sendData.actPeriodReqList)
          : []
          this.putBanner(sendData)
        }
      })
    },
    // 详情
    async getBannerDetail() {
      this.loading = true
      try {
        const { results } = await fhGetBannerDetail(this.bannerId)
        this.files.push({
          uid: '-1',
          name: 'image.png',
          status: 'done',
        ...
        // 传给组件的选择菜品
        this.selectedFood = results.foodTemplateNameList.reduce((acc, cur, index) => {
          const name = cur; const id = results.foodTemplateIdList[index]
          acc.push({ name, id })
          return acc
        }, [])
        const resultsData = JSON.parse(JSON.stringify(results))
        if (!resultsData.allDayFlag) {
          results.actPeriodReqList.forEach((ele, index) => {
            resultsData.actPeriodReqList[index] = [ele.startTime, ele.endTime]
          })
        }

        this.formData = resultsData
        // 传给店铺选择组件
        this.checkedStore = resultsData.storeIdList.reduce((acc, cur, index) => {
          const label = resultsData.storeNameList[index]
          const id = cur
          acc.push({ label, id })
          return acc
        }, [])

        this.formData.weekCycle = this.formData.weekCycle.split('').reverse()
        this.effectiveChoose = resultsData.weekCycle.reduce((acc, cur, index) => {
          if (cur === '1') {
            acc.push(index)
          }
          return acc
    ...
    // 编辑
    async putBanner(params) {
      this.loadingSave = true
      try {
        await fhPutBanner(this.bannerId, params)
    ...
    selectFoodFn(food, id) {
      ...
      this.formData.foodTemplateIdList = id
    ...
    selectStore(item, id) {
      ...
      this.formData.storeNameList = item
      this.$refs.form.validateField('storeIdList')
      this.formData.foodTemplateIdList = []
      this.selectedFood = []
    },
    ...
    chooseEffective(v, i) {
      this.formData.weekCycle[i] = v ? 1 : 0
    },
