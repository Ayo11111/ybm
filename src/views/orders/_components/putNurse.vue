 ...
 * @LastEditors: HJ
 * @LastEditTime: 2021-07-08 19:07:00
-->
<template>
  <a-modal
    :visible="visible"
      ...
      <div slot="info" slot-scope="scope">
        <div style="display: flex">
          ...
          <div class="infoText">
            ...
            <span>
              {{ scope.sex === 1 ? '男' : '女' }}
              {{ computedNurseAge(scope.birthday) }}
            </span>
          </div>
        </div>
      </div>
...
import { fhGetNurses } from '@/api/nurse'
import { fhPutOrderNurse } from '@/api/orders'
export default {
  components: {
    FilterSelect,
    Pagination
  ...
  methods: {
    async getNurses(params) {
      ...
      try {
        ...
        const { results: res, count } = results
        let newRes = []
        if (this.activeNurse) {
          newRes = res.filter(item => {
            return item.id !== this.activeNurse
          })
        } else {
          newRes = res
        }
