 ...
 * @version: v1.0
 * @Author: HJ
 * @Date: 2020-07-13 15:23:20
 * @LastEditors: LuoJie
 * @LastEditTime: 2022-05-16 18:03:23
-->
<template>
        ...
        <div class="base base-right">
          <el-row>
            ...
            >
              ...
              <a-button
                v-if="['retail', 'restaurant'].includes(type)"
                type="link"
                @click="value.openStatus = !value.openStatus"
              >
                {{ value.openStatus ? '收起' : '展开' }}商品信息
              </a-button>
            </el-col>
            <div v-show="value.openStatus">
          ...
          >
            ...
            <a-timeline style="margin: 15px">
              ...
              >
                <div v-if="value.refundStatus === 603">
                  <span>商家拒绝退款</span><br>
                  <span style="color: #999">
                    拒绝理由：{{ value.reason || '' }}
                  </span>
                </div>
                <div v-else>{{ time }}后自动退款</div>
  ...
  methods: {
    ...
    async putRefundPartial(params) {
      ...
      try {
        ...
        params.managerId = backUserId

        this.merchantType = ['retail', 'restaurant'].includes(this.type)
        ...
        if (valid) {
          ...
          try {
            ['retail', 'restaurant'].includes(this.type)
              ? await this.cancelOerder(this.rejectForm)
